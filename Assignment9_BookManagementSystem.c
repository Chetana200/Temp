...
int search_by_bookName(char []);
int search_by_authorName(char [] );
void test_search_by_bookName(char [],int );
void test_search_by_authorName(char [],int );
...
/*    int choice;
    ...
    FILE *ptr;
    ptr = fopen("Assignment9_book.txt", "r");
    int Expectedoutcome;
    char ch[20];
	if (ptr == NULL)
    {
...
int search_by_bookName(char bookName[30])
{
	int count=0;
...
int search_by_authorName(char authorName[30])
{
	int count=0;
	struct book *ptr=(struct book*)malloc(sizeof(struct book*));
	ptr=start_lib;
...
{
	if(search_by_bookName(bookName)==expectedOutcome)
	{
		printf("\nSuccessful\n");
	}
	else
...
{
	if(search_by_authorName(authorName)==expectedOutcome)
	{
		printf("\nSuccessful\n");
	}
	else
